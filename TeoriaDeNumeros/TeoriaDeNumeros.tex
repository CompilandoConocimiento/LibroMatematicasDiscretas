...
\chapter{Teoría de Congruencias}
    ...
    \section{Aplicaciones}
        ...
        % ==================================
        % ==   ENCONTRAR RESIDUO DE N   ====
        % ==================================
        \clearpage
        \subsection{Encontrar el Residuo en un Numerote ($a^b$)}

            Es muy común usar congruencias para encontrar el residuo de 
            un númerotote (generalmente denotado de la forma $a^b$) al
            divirlo entre alguna $n$.
            ...
            \begin{itemize}
                \item Aplica el algoritmo de la división y llega a $a=nq+r$, de aquí tienes
                    la proposición $a \equiv r \pmod{n}$

                \item Empieza a elevar proposición anterior hasta llegar a:

                    $a^k \equiv r^k \pmod{n}$ donde $r^k \equiv 1 \pmod{n}$


                \item Ya que sabemos que $1^n=1$ entonces ya nos podremos acerca mucho más,
                    ¿Pero cuanto?

                    Aplica el algoritmo de la división y llega a $b = kq' + r'$, por lo
                    tanto puedes escribir la proposición $a^{kq'} \equiv 1 \pmod{n}$

                \item Finalmente puedes también decir que $a^{r'} \equiv r^{r'} \pmod{n}$,
                por lo tanto si multiplicas las ultimas dos congruencias tenemos que
                $a^{kq'}a^{r'} \equiv (1)(r^{r'})$ esto es lo mismo que
                $a^{kq'+r'} \equiv r^{r'} \pmod{n}$

                Por lo tanto $a^{b} \equiv r^{r'} \pmod{n}$. Donde $r^{r'} \pmod {n}$ es 
                nuestra respuesta.
